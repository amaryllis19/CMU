...
for i = 1:size(season,1)
    ...
    % apply the bayes classifier on test data 
     ...
     P_tst_y(1,1) = P_umbrella_y * P_yesterday_tst_y * P_daybefore_tst_y * P_cloud_tst_y
     P_tst_n(1,1) = P_umbrella_n * P_yesterday_tst_n * P_daybefore_tst_n * P_cloud_tst_n
     
     if(P_tst_y(1,1) > P_tst_n(1,1))
              result = 'y'
          else
              result = 'n'
     end
      
     acc(1) = acc(1) + strcmp(result,umbrella_tst)
     
     P_tst_y(1,2) = P_umbrella_y * P_season_tst_y  * P_daybefore_tst_y * P_cloud_tst_y
     P_tst_n(1,2) = P_umbrella_n * P_season_tst_n  * P_daybefore_tst_n * P_cloud_tst_n
     
     if(P_tst_y(1,2) > P_tst_n(1,2))
              result = 'y'
          else
              result = 'n'
     end
     
     acc(2) = acc(2) + strcmp(result,umbrella_tst)
     
     P_tst_y(1,3) = P_umbrella_y * P_season_tst_y * P_yesterday_tst_y  * P_cloud_tst_y
     P_tst_n(1,3) = P_umbrella_n * P_season_tst_n * P_yesterday_tst_n  * P_cloud_tst_n
     
     if(P_tst_y(1,3) > P_tst_n(1,3))
              result = 'y'
          else
              result = 'n'
     end
     
     acc(3) = acc(3) + strcmp(result,umbrella_tst)
     
     P_tst_y(1,4) = P_umbrella_y * P_season_tst_y * P_yesterday_tst_y * P_daybefore_tst_y 
     P_tst_n(1,4) = P_umbrella_n * P_season_tst_n * P_yesterday_tst_n * P_daybefore_tst_n 
     
     if(P_tst_y(1,4) > P_tst_n(1,4))
              result = 'y'
          else
              result = 'n'
     end
      
     acc(4) = acc(4) + strcmp(result,umbrella_tst)
...
[min index] = max(acc)    % index will give the best 3 features
avg_acc = acc/length(season);